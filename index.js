...
function nodeConfig(config) {
    ...
    var dojoBasePath = path.resolve(path.dirname(require.resolve('dojo')));
    // reset some configuration switches with node-appropriate values
    var nodeConfig = {
        baseUrl: dojoBasePath,
        ...
        loaderPatch: {
            ...
            eval: function (__text, __urlHint) {
                return vm.runInContext(__text, config.context, __urlHint);
            },

            injectUrl: function (url, callback) {
                try {
                    vm.runInContext(fs.readFileSync(url, "utf8"), config.context, url);
                    callback();
                } catch (e) {
            ...
            getText: function (url, sync, onLoad) {
                ...
                onLoad(fs.readFileSync(dojoBasePath + '/' + url, "utf8"));
            }
        }
    };
    for (p in nodeConfig) {
